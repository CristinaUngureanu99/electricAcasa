...
import type { Product, ShippingMethod } from '@/types/database';
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const clampAndClean = useCallback(
    async (
    ...
    async (productId: string, quantity = 1): Promise<boolean> => {
      ...
      if (userId) {
        ...
        if (existing) {
          ...
          const { error } = await supabase
            .from('cart_items')
            .insert({ user_id: userId, product_id: productId, quantity: newQty });
          if (error) return false;
        }
      }
  ...
  const baseCost =
    shippingMethod === 'easybox' ? site.shipping.easyboxCost : site.shipping.fixedCost;
  const shippingCost =
    cartItems.length === 0 ? 0 : subtotal >= site.shipping.freeThreshold ? 0 : baseCost;
