...
export default async function CategoriePage({ params, searchParams }: Props) {
  ...
  // Determine which category IDs to query products from
  const subFilter = sp.sub || ''; // subcategory slug filter
  const activeSub = subs.find((s) => s.slug === subFilter);

  let productCategoryIds: string[];
  if (isLeaf) {
    productCategoryIds = [cat.id];
  } else if (activeSub) {
    productCategoryIds = [activeSub.id];
  } else {
    productCategoryIds = subs.map((s) => s.id);
  }

  // Count products per subcategory (for sidebar counts)
  ...
  // Fetch products + filters for ALL categories (leaf and non-leaf)
  ...
  // Get unique brands for filter (from all subcategory products)
  const { data: brandRows } = await supabase
    .from('products')
    .select('brand_name')
    .in('category_id', isLeaf ? [cat.id] : subs.map((s) => s.id))
    .eq('is_active', true)
    .neq('brand_name', '');
            ...
            {subs.length > 0 && (
              <div>
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    Toate{' '}
                    {subCounts ? `(${Object.values(subCounts).reduce((a, b) => a + b, 0)})` : ''}
                  </Link>
                  {subs.map((sub) => (
                    <Link
                      key={sub.id}
                    ...
                    >
                      {sub.name}
                      {subCounts[sub.id] > 0 && (
                        <span className="text-xs text-gray-400 ml-1">({subCounts[sub.id]})</span>
                      )}
                    </Link>
                  ))}
                </div>
