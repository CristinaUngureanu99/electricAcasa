...
export default function CheckoutPage() {
  ...
  function getFieldError(field: string, value: string): string {
    ...
    switch (field) {
      case 'name':
        return v.length > 0 && v.length < 3 ? 'Minim 3 caractere' : '';
      case 'phone':
        return v.length > 0 && !/^(\+?40|0)[237]\d{8}$/.test(v.replace(/[\s\-().]/g, ''))
          ? 'Format invalid (ex: 07xx xxx xxx)'
          : '';
      case 'postalCode':
        return v.length > 0 && !/^\d{6}$/.test(v) ? 'Codul postal are 6 cifre' : '';
      case 'street':
        return v.length > 0 && v.length < 5 ? 'Minim 5 caractere' : '';
      case 'city':
        return v.length > 0 && v.length < 2 ? 'Minim 2 caractere' : '';
      case 'county':
        return v.length > 0 && v.length < 2 ? 'Minim 2 caractere' : '';
      default:
        return '';
        ...
        <Card>
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Ai deja o comanda card in curs de procesare. Alege ce vrei sa faci:
            </p>
            <div className="flex flex-col gap-3">
              <Button onClick={handleResume} loading={submitting}>
                Reia plata
              </Button>
              <Button variant="danger" onClick={handleCancelPending} loading={submitting}>
                Anuleaza comanda
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6">
            <Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input
                  ...
                  onBlur={() => validateField('county', county)}
                  error={errors.county}
                  required
                />
                <Input
            ...
            <Card>
              ...
              <div className="space-y-3">
                ...
                <label
                  className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'ramburs' ? 'border-primary bg-primary/5 shadow-sm shadow-primary/10' : 'border-gray-200 hover:border-gray-300'}`}
                >
                  <input
                    type="radio"
                    name="payment"
                  ...
                  <Banknote
                    size={20}
                    className={paymentMethod === 'ramburs' ? 'text-primary' : 'text-gray-400'}
                  />
                  <div>
