...
export default async function ConfirmarePage({ searchParams }: { searchParams: Promise<{ order_id?: string }> }) {
  ...
  const statusLabel = o.status === 'confirmed' ? 'Confirmata' : o.status === 'pending' ? 'In asteptare plata' : o.status;
      ...
      <Card>
        <div className="space-y-4">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium">{o.payment_method === 'card' ? 'Card bancar' : 'Ramburs'}</span>
          </div>
